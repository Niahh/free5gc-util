...
// configFallback mirrors the config getter every NF passes.
...
func TestTick(t *testing.T) {
	// The NRF assigns currentTimer at registration, reregisterTimer on re-registration.
	const (
		currentTimer    = 10
		reregisterTimer = 30
	)
...
func TestZeroFallbackStillHeartbeats(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Without the DefaultTimer clamp this panics in NewTicker(0).
...
func TestLoopAdoptsNewInterval(t *testing.T) {
	// The NRF assigns initialTimer at registration, adoptedTimer on the first heartbeat.
	const (
		initialTimer = 1
		adoptedTimer = 2
	)
	...
	synctest.Test(t, func(t *testing.T) {
		...
		// Read them only once the goroutine that owns them has exited.
		cancel()
		waitStopped(t, runner)
		wg.Wait()
...
// Wait may not return while a heartbeat PATCH is in flight, even after shutdown.
...
// waitStopped must run inside a synctest bubble: the deadlock it guards against shows
// up as a blocked bubble, not as a timeout.
